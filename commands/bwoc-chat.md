...
Note: `bwoc chat` execs an interactive backend CLI and takes over the terminal. Inside
Claude Code's non-interactive Bash tool prefer `/bwoc:run` for a one-shot task; surface
`bwoc chat` to the user for a hands-on session.
