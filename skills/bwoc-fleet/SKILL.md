---
name: bwoc-fleet
description: Coordinate the BWOC agent fleet from Claude Code. Use when the user wants to list/inspect BWOC agents, delegate or send work to a fleet member, run a headless task on an agent, manage Saṅgha teams or their shared task list, or read workspace deep-memory. Wraps the `bwoc` CLI (shell-out, no server).
---
...
- **`@agent-<id>`** — the re-exported sub-agents (see `agents/`) let Claude Code delegate
  to a fleet member as a native sub-agent; each sub-agent itself shells out to `bwoc`.
